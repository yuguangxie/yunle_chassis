...
| `scu_control_max_steering_angle_deg` | `27.0` | 底盘实际最大转角；用于将 `/control/scu_control_command` 的前/后转向角度值换算成 8 位补码 raw，也用于将 SAS/CCU 转角反馈编码值换算成实际角度。 |
...
3. 每个通道保存固定的远端 IP 和各自的远端 UDP 目标端口。
...
| 225 | RX | UDP 接收事件触发 | `dbc_protocol.cpp:56`, `chassis_driver_node.cpp:321` | `decodeSignal`, `publishDecoded` | `SAS_Front_Angle`, `SAS_Rear_Angle` | `0|16`, `24|16` | 0.1 | 0 | 按 `编码值 / 120 * 最大转角` 换算后的 deg | `/feedback/sas_angle` | signed 16-bit；bits 16-23 有空隙。 |
