...
| `scu_drive_mode_request` | 驾驶模式请求 | `SCU_Drive_Mode_Request` | 当 ROS 值为 `0` 时默认下发 `1`，即自动驾驶模式；`1` 和 `3` 保留为有效输入；其他值回退到默认值。 |
| `scu_steering_angle_front` | 前轮目标角度，单位 deg | `SCU_Steering_Angle_Front` | 按配置最大转角换算为 8 位补码 raw。 |
| `scu_steering_angle_rear` | 后轮目标角度，单位 deg | `SCU_Steering_Angle_Rear` | 按配置最大转角换算为 8 位补码 raw。 |
| `scu_target_speed` | 目标速度大小，单位 km/h | `SCU_Target_Speed` | 使用绝对值下发；超过配置最大速度则视为无效数据并拒发本帧。 |
...
| `scu_control_max_target_speed_kmh` | `15.0` | `/control/scu_control_command` 接受的最大目标速度。超过该值的命令不会下发。 |
| `scu_control_default_drive_mode_request` | `1` | 当 ROS 消息中 `scu_drive_mode_request=0` 时使用的默认驾驶模式。当前默认 `1=自动驾驶模式请求`。 |
...
当前驱动将 `scu_target_speed` 作为速度大小处理：

1. 先取绝对值，确保下发给 `SCU_Target_Speed` 的物理值为正。
2. 如果速度不是有限数，或超过 `scu_control_max_target_speed_kmh`，则认为该 ROS 命令无效，本次不发送 CAN ID `0x121`。
...
|---|---|---|---|---|
| 驾驶模式默认值 | ROS 值为 `0` 时默认下发 `1`，即自动驾驶模式 | 章节 3.3 写明 `SCU_Drive_Mode_Request=1` 为自动驾驶模式请求，`3` 为遥控器模式请求，并称遥控器模式为默认模式请求 | DBC `VAL_ 289 SCU_Drive_Mode_Request` 当前写的是 `0 Manual`、`1 Remote`、`2 Auto`、`3 Reserved` | 文档与 DBC 枚举不一致。本次按用户要求和 2026 使用说明实现。 |
...
| 目标速度最大值 | 可配置，默认 `15 km/h`；超限拒发 | 文档写 JD/WD 出厂限速默认 10 km/h，TD 默认 15 km/h；字段范围仍为 `0-51 km/h` | DBC 范围是 `0-51.1 km/h`，原代码由 DBC clamp | 本次按用户要求用 15 km/h 默认上限，不再把超限值 clamp 后发送。 |
| 速度符号 | ROS 输入取绝对值后下发，方向由档位控制 | 文档示例前进/后退均发送正速度 `3 km/h`，通过 D/R 档区分方向 | 原代码直接编码 ROS 输入，负数会被 unsigned DBC 信号 clamp 到 0 | 本次按协议示例改为正速度语义。 |
...
| 文件 | 修改内容 |
|---|---|
| `chassis_interfaces/msg/ScuControlCommand.msg` | 增加档位/驾驶模式常量和字段语义注释。 |
...
| `chassis_driver/src/control_command_bridge.cpp` | 在 `/control/scu_control_command` callback 中增加档位校验、默认驾驶模式、角度换算和速度超限拒发。 |
