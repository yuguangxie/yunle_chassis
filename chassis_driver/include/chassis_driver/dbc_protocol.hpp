 ...
 * @brief 基于代码内报文定义的静态 DBC 信号编解码工具。
 */
class DbcProtocol
{
public:
  /** @brief Access frame-ID keyed message definitions. */
  /** @brief 获取按帧 ID 索引的报文定义。 */
   ...
   * @brief 从 CAN 帧中解析一个物理信号值。
   * @return 匹配成功时返回信号值，否则返回 std::nullopt。
   */
