...
public:
   ...
   * @brief 将 UDP payload 字节解码为 CAN 帧。
   * @param payload 从以太网转 CAN 设备接收到的原始字节。
   * @param channel 写入解码帧的逻辑通道 ID。
   * @param had_trailing_bytes 当 payload 长度不是单条记录整数倍时置为 true。
   * @param trailing_bytes_count payload 尾部被忽略的字节数。
   * @return 解码得到的 CAN 帧列表。
   */
