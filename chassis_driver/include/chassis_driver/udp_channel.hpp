 ...
 * @brief 固定远端地址通信使用的轻量 UDP socket 封装。
 */
class UdpChannel
{
public:
   ...
   * @brief 打开 UDP socket，绑定本地端口并配置远端目标。
   * @return 当 socket 创建、绑定和端点配置均成功时返回 true。
   */
   ...
   * @param out_payload 接收到的 payload 字节。
   ...
   * @brief 向已配置的远端端点发送一个 UDP 数据报。
   * @param payload 待发送的 payload 字节。
   * @return 完整 payload 发送成功时返回 true。
   */
  bool send(const std::vector<uint8_t> & payload);

  /** @brief Idempotently close socket and clear state. */
  /** @brief 幂等关闭 socket 并清空通道状态。 */
  void close();
